...
class EcoTracker:
    ...
    def get_stats(self) -> dict:
        return {
            ...
            'total_impact': self.impact_manager.calculate_total_impact(),
            'weekly_impact': self.impact_manager.get_weekly_impact()
        }
...
class EnvironmentalImpactManager:
    ...
    def calculate_total_impact(self) -> ImpactData:
        try:
            with self.db._get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT action, COUNT(*) FROM actions GROUP BY action")
                ...
                for action, count in action_counts:
                    if action in self.impact_values:
                        total_co2 += self.impact_values[action]["co2"] * count
                        total_gas += self.impact_values[action]["gas"] * count
    ...
    def get_weekly_impact(self) -> ImpactData:
        try:
            with self.db._get_connection() as conn:
                cursor = conn.cursor()
                one_week_ago = (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d %H:%M:%S")
                cursor.execute(
                    "SELECT action, COUNT(*) FROM actions WHERE timestamp >= ? GROUP BY action", 
                    (one_week_ago,)
                )
                action_counts = cursor.fetchall()
